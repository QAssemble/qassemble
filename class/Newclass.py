...
class Crystal(object): # chemical potential object, num of electron 
    def __init__(self,latt : list, basisposition : dict, ns : int, soc : bool, rkgrid : list, orboption : list, N : float, supercell : list = [1,1,1], impdict : dict = None):
...
class SigmaFock(FLatStc):
    ...
    def __init__(self, crystal: Crystal,occr = None, vbare : object = None): # green -> occ
        super().__init__(crystal)
        self.fr = None
        self.fk = None
        self.fdyn = None
...
class BLatDyn(object):
    ...
    def Quad2Double(self,matin : np.ndarray)->np.ndarray:
        ...
        for ift in range(nft):
            for irk in range(nrk):
                for ks in range(ns):
                    for js in range(ns):
                        matout[:,:,js,irk,ift] = self.crystal.Quad2Double(matin[:,:,:,:,js,ks,irk,ift])
    ...
    def Double2Quad(self,matin : np.ndarray)->np.ndarray:
        ...
        matout = np.zeros((norb,norb,norb,norb,ns,ns,nrk,nft),dtype=complex,order='F')
    ...
    def Cal(self):
                    ...
                    for js in range(ns):
                        for ks in range(ns):
                            for iorbc, jorbc,korbc,lorbc in itertools.product(list(range(norbc)),list(range(norbc)),list(range(norbc)),list(range(norbc))):
                                if js==ks:
                                    tempmat[iorbc,lorbc,jorbc,korbc,js,ks,irk,itau] = gmrt[jorbc,iorbc,js,irk,itau]*grt[korbc,lorbc,ks,irk,itau]
        else:
            if self.crystal.soc == True:
                C = 1
                for itau in range(ntau):
                    for irk in range(nrk):
                        for iorbc, jorbc,korbc,lorbc in itertools.product(list(range(norbc)),list(range(norbc)),list(range(norbc)),list(range(norbc))):
                            tempmat[iorbc,lorbc,jorbc,korbc,0,0,irk,itau] = gmrt[jorbc,iorbc,0,irk,itau]*grt[korbc,lorbc,0,irk,itau]*C
            else:
                C = 2
                for itau in range(ntau):
                    for irk in range(nrk):
        ...
        for itau in range(ntau):
                for irk in range(nrk):
                    for js in range(ns):
                        for ks in range(ns):
                            polrt[:,:,js,ks,irk,itau] = self.crystal.Quad2Double(tempmat[:,:,:,:,js,ks,irk,itau])
        self.polrt = polrt
...
class WLat(BLatDyn):
    ...
    def Cal(self): # calculate W and Wc
        ...
        nfreq = len(self.ft.nu)
        ####### Initialization #######
        tempmat = np.zeros((norbc*norbc,norbc*norbc,ns,ns,nk,nfreq),dtype=complex,order='F')
        ...
        for ifreq in range(nfreq):
            for ik in range(nk):
                for js in range(ns):
                    for ks in range(ns):
                        polcomp[:,:,js,ks,ik,ifreq] = self.crystal.Double2Full(self.pol.polkf[:,:,js,ks,ik,ifreq])
                        vcomp[:,:,js,ks,ik,ifreq] = self.crystal.Double2Full(vdyn[:,:,js,ks,ik,ifreq])
        ...
        for ifreq in range(nfreq):
            for ik in range(nk):
                for js in range(ns):
                    for ks in range(ns):
                        wkf[:,:,js,ks,ik,ifreq] = self.crystal.Full2Double(tempmat[:,:,js,ks,ik,ifreq])
        self.wkf = wkf
...
class BLatStc(object):
    ...
    def Quad2Double(self,matin : np.ndarray)->np.ndarray:
        ...
        for irk in range(nrk):
            for ks in range(ns):
                for js in range(ns):
                    matout[:,:,js,irk] = self.crystal.Quad2Double(matin[:,:,:,:,js,ks,irk])
    ...
    def Double2Quad(self,matin : np.ndarray)->np.ndarray:
        ...
        matout = np.zeros((norb,norb,norb,norb,ns,ns,nrk),dtype=complex,order='F')
...
class VBare(BLatStc):
    ...
    def Cal(self):
        ...
        tempmat = np.zeros((norb,norb,ns,ns,rkgrid[0],rkgrid[1],rkgrid[2]),dtype=complex,order='F')
                ...
                for ks in range(ns):
                    for m1 in m:
                        for m2 in m:
                            for m3 in m:
                                for m4 in m:
                                    v[m1+l,m2+l,m3+l,m4+l,js,ks] += f*self.AngularIntegral(l,k,m1,m2,m3,m4)
        if sc == 'c':
            for js in range(ns):
                for ks in range(ns):
                    tempmat = v[:,:,:,:,js,ks]
...
class CorrelationFunction(object):
    ...
    def HartreeFockH(self,itermax : int,mix : float, T : float, size : int, hoppinglist : list, onsitelist : list, option : dict = None, intamp : list = None):
    ...
    def HartreeFock(self,itermax : int,mix : float, T : float, size : int, hoppinglist : list, onsitelist : list, option : dict = None, intamp : list = None):
