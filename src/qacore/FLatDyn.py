...
class FLatDyn(object):
    ...
    def K2R_new(self, matk: np.ndarray, rkgrid: list = None) -> np.ndarray:
        rkvec = self.crystal.kpoint
        if rkgrid is None:
            rkgrid = self.crystal.rkgrid
        ...
        # Orbital -> atom index (cache once per call)
        orb2atom = np.empty(norb, dtype=np.int64)
        for iorb in range(norb):
            a, _ = self.crystal.FAtomOrb(iorb)
            orb2atom[iorb] = a

        basisf = np.asarray(self.crystal.basisf)  # (natom, 3)

        # Memory-efficient phase: k·(ri-rj) = (k·ri) - (k·rj)
        basis_orb = basisf[orb2atom]                  # (norb, 3)
        ...
        # Make phase axes match (norb, norb, nrk)
        phase_T = np.transpose(phase, (1, 2, 0))      # (norb, norb, nrk)

        tempmat = matk.copy()
        # Broadcast to (norb, norb, ns, nrk, nft) via (norb, norb, 1, nrk, 1)
        tempmat *= phase_T[:, :, None, :, None]
    ...
    def R2K_new(self, matr: np.ndarray) -> np.ndarray:
        ...
        # 1) Fourier transform first
        matk = QAFort.fourier.flatdyn_r2k(rkgrid, matr)  # (norb,norb,ns,nrk,nft)

        # 2) Precompute orbital -> atom index (avoid repeated FAtomOrb)
        #    Cache on self if called often
        faorb2atom = np.empty(norb, dtype=np.int64)
        for iorb in range(norb):
            a, _ = self.crystal.FAtomOrb(iorb)
            faorb2atom[iorb] = a
        ...
        #    k·(ra-rb) = (k·ra) - (k·rb)
        basis_orb = basisf[faorb2atom]                 # (norb, 3)
        kb = rkvec[:nrk] @ basis_orb.T                 # (nrk, norb)
        k_dot_delta = kb[:, :, None] - kb[:, None, :]  # (nrk, norb, norb)
        ...
        # 4) Apply to all js and ift at once via broadcasting
        #    matk: (norb,norb,ns,nrk,nft)
        #    phase broadcast: (norb,norb,1,nrk,1)
        matk *= phase_T[:, :, None, :, None]
...
class GreenInt(FLatDyn):
    ...
    def UpdateMu(self) -> np.ndarray:
        ...
        # self.rf = self.K2R_new(self.kf)
        # self.rt = self.K2R_new(self.kt)
...
class GreenInt_new(FLatDyn):
    ...
    def __init__(self, crystal: Crystal, ft: FTGrid, greenbare : np.ndarray = None, sigmah : np.ndarray = None, sigmaf : np.ndarray = None, sigmagwc : np.ndarray = None, hdf5file : str = 'glob.h5', group : str = None) -> object:
        ...
        print("GreenInt.CalMu0 code      - ",round(tiem_delta,5),'seconds')

        start = time.time()
        self.SearchMu()
        end = time.time()
        tiem_delta = end-start
        print("GreenInt.SearchMu code      - ",round(tiem_delta,5),'seconds')
    ...
    def CalMu0(self):
        ...
        tiem_delta = end-start
        print("GreenInt.F2T code      - ",round(tiem_delta,5),'seconds')

        start = time.time()
        self.grfmu0 = self.K2R_new(self.gkfmu0)
        end = time.time()
        tiem_delta = end-start
        print("GreenInt.K2R_new (F domain) code      - ",round(tiem_delta,5),'seconds')

        start = time.time()
        self.grtmu0 = self.K2R_new(self.gktmu0)
        end = time.time()
        tiem_delta = end-start
        print("GreenInt.K2R_new (T domain) code      - ",round(tiem_delta,5),'seconds')
    ...
    def UpdateMu(self) -> np.ndarray:
        ...
        tiem_delta = end-start
        print("GreenInt.ChemEmbedding code      - ",round(tiem_delta,5),'seconds')

        start = time.time()
        gkfnew = self.Dyson(self.gkfmu0,-chem)
        end = time.time()
        tiem_delta = end-start
        print("GreenInt.Dyson code      - ",round(tiem_delta,5),'seconds')
        ...
        tiem_delta = end-start
        print("GreenInt.K2R (F domain) code      - ",round(tiem_delta,5),'seconds')

        start = time.time()
        self.rt = self.K2R_new(self.kt)
        end = time.time()
        tiem_delta = end-start
        print("GreenInt.K2R (T domain) code      - ",round(tiem_delta,5),'seconds')
        # self.rf = self.K2R_new(self.kf)
        # self.rt = self.K2R_new(self.kt)
        print("Chemical potential shift finish")
        self.Occ()
    ...
    #     end = time.time()
    #     tiem_delta = end-start
    #     print("GreenInt.NumOfE code      - ",round(tiem_delta,5),'seconds')
    ...
    #     end = time.time()
    #     tiem_delta = end-start
    #     print("GreenInt.scipy.optimize.brentq code      - ",round(tiem_delta,5),'seconds')

    #     start = time.time()
    #     self.UpdateMu()
    #     end = time.time()
    #     tiem_delta = end-start
    #     print("GreenInt.UpdateMu code      - ",round(tiem_delta,5),'seconds')
    #     return None
    ...
    def SearchMu(self):
        ...
        wmax = float(self.ft.omega[-1])
        mumin = -0.6 * wmax
        mumax =  +0.6 * wmax
        print(f"minimum : {mumin}, maximum : {mumax}")

        # ---- Cache wrapper to avoid repeated expensive evaluations ----
        cache = {}
        def f(mu):
            mu = float(mu)
            if mu in cache:
        ...
        print("GreenInt.NumOfE code      - ", round(end - start, 5), "seconds")
        ...
        # ---- Faster bracketing solver: TOMS748 ----
        start = time.time()
        res = scipy.optimize.root_scalar(
            f,
            bracket=(mumin, mumax),
            method="toms748",
            xtol=1.0e-6,   # your original target
            # rtol omitted (SciPy requires rtol >= ~eps; leaving it default is safe)
            maxiter=100
        )

        if not res.converged:
            raise RuntimeError(
                f"Mu search did not converge. iterations={res.iterations}, flag={res.flag}"
            )

        self.mu_old = res.root
        print("Finding chemical potential finish")
        end = time.time()
        print("GreenInt.root_scalar(toms748) code - ", round(end - start, 5), "seconds")
        # print("function calls:", res.function_calls)  # useful for profiling

        start = time.time()
        self.UpdateMu()
        end = time.time()
        print("GreenInt.UpdateMu code      - ", round(end - start, 5), "seconds")
        return None
...
class SigmaGWC_new(FLatDyn):
    ...
    def Cal(self)->np.ndarray: #SigmaGWC
        ...
        # --- Precompute mappings ONCE (outside the tau/r loops) ---
        # A[iorb] = iorbc1,  B[iorb] = iorbc4
        A = np.empty(norb, dtype=np.int64)
        B = np.empty(norb, dtype=np.int64)
        for iorb in range(norb):
            a, (m1, m4) = self.crystal.BAtomOrb(iorb)
            A[iorb] = self.crystal.FIndex([a, m1])
            B[iorb] = self.crystal.FIndex([a, m4])

        # C[jorb] = iorbc2,  D[jorb] = iorbc3
        C = np.empty(norb, dtype=np.int64)
        D = np.empty(norb, dtype=np.int64)
        for jorb in range(norb):
            b, (m3, m2) = self.crystal.BAtomOrb(jorb)
            C[jorb] = self.crystal.FIndex([b, m2])
            D[jorb] = self.crystal.FIndex([b, m3])

        # Check if mapping indices are unique (fast path possible)
        A_unique = (np.unique(A).size == A.size)
        C_unique = (np.unique(C).size == C.size)

        # Precompute ix objects for speed
        ix_G = np.ix_(B, D)   # for G[ B[:,None], D[None,:], ...]
        ix_out = np.ix_(A, C) # for crtau[ A[:,None], C[None,:], ...]
        ...
        for itau in range(ntau):
            for ir in range(nr):
                for s in range(ns):
                    # Correct 5D slicing:
                    # G is assumed shape (nF, nF, ns, nr, ntau)
                    # Wc assumed shape (norb, norb, ns, ns, nr, ntau)
                    # crtau assumed shape (nF, nF, ns, nr, ntau)

                    # G_slice[p,q] = G[B[p], D[q], s, ir, itau]  -> (norb, norb)
                    G_slice = G[ix_G[0], ix_G[1], s, ir, itau]

                    # W_slice[p,q] = Wc[p,q,s,s,ir,itau]        -> (norb, norb)
                    W_slice = Wc[:, :, s, s, ir, itau]

                    contrib = -G_slice * W_slice  # (norb, norb)

                    if A_unique and C_unique:
                        # FAST PATH: direct block add (no scatter kernel)
                        crtau[ix_out[0], ix_out[1], s, ir, itau] += contrib
                    else:
                        # SAFE FALLBACK: scatter-add (slower, but correct if repeats exist)
                        # This is the operation:
                        # crtau[A[p], C[q], s, ir, itau] += contrib[p,q]
                        np.add.at(crtau[:, :, s, ir, itau], (A[:, None], C[None, :]), contrib)
                
                                        

        cktau = self.R2K_new(crtau)
        ckfreq = self.T2F(cktau)
        crfreq = self.T2F(crtau)
