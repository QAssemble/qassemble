...
!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine FLocDyn_T2F_Test2(norb,ns,ntau,tau,ftau,nf,freq,ff)
    ...
    double precision :: pi, beta, taurad_finu((-ntau):(ntau-1)),prefac
    complex*16 :: ff_finu((-2*nf+1):(2*nf-1)), ftau_finu((-ntau):(ntau-1))
    integer*8 :: ntau_finu, nf_finu
    integer*8, allocatable :: null
    ...
    ff=0.0d0
    prefac = 5.0d0 * (16.d0 * beta)**(1.0/5.0) / real(ntau)
        ...
        do is=1, ns
          ...
          do itau=0, ntau-1
            if (itau .lt. ntau/2) then
              ftau_finu(itau)=ftau(iorb,jorb,is,itau)*prefac*tau(itau)**(4.0/5.0)
            else
              ftau_finu(itau)=ftau(iorb,jorb,is,itau)*prefac*(beta-tau(itau))**(4.0/5.0)
            endif
            ftau_finu(itau-ntau)=-ftau_finu(itau)
          enddo
          call finufft1d1(ntau_finu,taurad_finu,ftau_finu,1, 1.0d-12, nf_finu,ff_finu,null,ierr)
      ...
      do jorb=1, norb
        do is=1, ns
          do js=1, ns
            ...
            ! enddo
            do ifreq = 0, 2*nf-1
              if (modulo(ifreq,2) .eq. 0) then
                ff(iorb,jorb,is,js,(ifreq)/2) = ff_finu(ifreq)/2.0d0
              endif
            enddo
          enddo
        enddo
...
!      print *, beta
      ...
      prefac = 5.0d0*(16.0d0 * beta)**(1.0/5.0) / real(ntau)

      do itau=0, ntau-1
         if (itau .lt. ntau/2) then
!            print *, 'itau < ntau/2'
             w(itau) = prefac*tau(itau)**(4.0/5.0)
             print *, itau, tau(itau), w(itau)
         else
!            print *, 'itau > ntau/2'
             w(itau) = prefac*(beta-tau(itau))**(4.0/5.0)
             print *, itau, tau(itau), w(itau)
         endif
      enddo
  ...
  subroutine BLatDyn_T2F_Test(norb,ns,nk,ntau,tau,ftau,nf,freq,ff,w)
    implicit none
    integer, intent(in) :: norb,ns,nk,ntau,nf
    double precision, intent(in) :: tau(0:(ntau-1)), freq(0:(nf-1)),w(0:(ntau-1))
    complex*16, intent(in) :: ftau(norb,norb,ns,ns,nk,0:(ntau-1))
    ...
    ff=0.0d0
    do ik=1, nk
      call BLocDyn_T2F_Test(norb,ns,ntau,tau,ftau(:,:,:,:,ik,:),nf,freq,ff(:,:,:,:,ik,:),w)
    enddo

  end subroutine BLatDyn_T2F_Test


  ! subroutine FLatDyn_R2K_test(norb,ns,nr,nk,nft,r,k,matr, matk)
  !   implicit None
  !   integer, intent(in) :: norb,ns,nr,nk,nft
  !   real, intent(in) :: r(nr,3), k(nk,3)
  !   complex*16, intent(in) :: matr(norb,norb,ns,nr,0:(nft-1))
  !   complex*16, intent(out) :: matk(norb,norb,ns,nk,0:(nft-1))

  !   integer :: ift, iorb, jorb, is, ir, ik
  !   double precision :: pi
  !   complex*16 :: tempmat(nr), ai, tempval
  ...
  !           do ik = 1, nk
  !             do ir = 1, nr
  !               tempval = tempval+matr(iorb,jorb,is,ir,ift)*cdexp(-2.0d0*ai*pi*dot_product(k(ik,:),r(ir,:)))
  !             enddo
  !             matk(iorb,jorb,is,ik,ift) = tempval
  !           enddo
  !         enddo
  !       enddo
  !     enddo
  ...
  !   end subroutine FLatDyn_R2K_test

  subroutine FPathStc_R2K(norb,ns,nr,nk,r,k,matr,matk)
      implicit None
      integer, intent(in) :: norb,ns,nr,nk
      real*8, intent(in) :: r(nr,3), k(nk,3)
      complex*16, intent(in) :: matr(norb,norb,ns,nr)
      complex*16, intent(out) :: matk(norb,norb,ns,nk)

      integer :: iorb, jorb, is, ir, ik
      double precision :: pi
      complex*16 :: tempmat(nr), ai, tempval
         ...
         do is = 1, ns
            do iorb = 1, norb
               do jorb = 1, norb
                  tempval = 0.0d0
                  do ir = 1, nr
                     tempval = tempval + matr(iorb,jorb,is,ir)*cdexp(-2.0*ai*pi*dot_product(k(ik,:),r(ir,:)))
                  enddo
                  matk(iorb,jorb,is,ik) = tempval
               enddo
            enddo
         enddo
     enddo

  end subroutine FPathStc_R2K
  
  subroutine FPathStc_K2R(norb, ns, nr, nk, r, k, matk, matr)
        implicit None
        integer, intent(in) :: norb, ns, nr, nk
        real*8, intent(in) :: r(nr, 3), k(nk, 3)
        complex*16, intent(in) :: matk(norb,norb,ns,nk)
        complex*16, intent(out) :: matr(norb,norb,ns,nr)

        integer :: iorb, jorb, is, ir, ik
        double precision :: pi 
        complex*16 :: tempmat(nk), ai, tempval 
            ...
            do is = 1, ns
                do iorb = 1, norb 
                    do jorb = 1, norb
                        tempval = 0.0d0
                        do ik = 1, nk   
                            tempval = tempval + matk(iorb,jorb,is,ik)*cdexp(2.0*ai*pi*dot_product(k(ik,:),r(ir,:)))
                        enddo
                        matr(iorb,jorb,is,ir) = matr(iorb,jorb,is,ir) + tempval / nk
                    enddo
                enddo
            enddo
        enddo

  end subroutine FPathStc_K2R

  subroutine FPathStc_Slab(norb, ns, nz, nkx, nky, nkz, z, k, matk, matk_slab)
    implicit None
    integer, intent(in) :: norb, ns, nz, nkx, nky, nkz
    real*8, intent(in) :: k(nkx, nky, nkz, 3), z(nz, nz)
    complex*16, intent(in) :: matk(norb, norb, ns, nkx, nky, nkz)
    complex*16, intent(out) :: matk_slab(norb, norb, ns, nkx, nky, nz, nz)

    integer :: iorb, jorb, is, iz1, iz2, ikx, iky, ikz
    double precision :: pi 
    complex*16 :: ai, tempval 
            ...
            do jorb = 1, norb
              do ikx = 1, nkx
                do iky = 1, nky
                  tempval = 0.0d0
                  do ikz = 1, nkz
                    tempval = tempval + matk(iorb, jorb, is, ikx, iky, ikz)*cdexp(2.0*ai*pi*k(1, 1, ikz,3)*z(iz1,iz2))
                  enddo
                  matk_slab(iorb, jorb, is, ikx, iky, iz1, iz2) = matk_slab(iorb, jorb, is, ikx, iky, iz1, iz2) + tempval / nkz
                enddo
              enddo
            enddo
          enddo
  ...
  end subroutine FPathStc_Slab

  subroutine FPathDyn_R2K(norb,ns,nr,nk,nfreq,r,k,matr,matk)
      implicit None
      integer, intent(in) :: norb,ns,nr,nk,nfreq
      real*8, intent(in) :: r(nr,3), k(nk,3)
      complex*16, intent(in) :: matr(norb,norb,ns,nk,nfreq)
      complex*16, intent(out) :: matk(norb,norb,ns,nk,nfreq)
      integer :: ifreq

      matk = 0.0d0
      do ifreq = 0, nfreq-1
          call FPathStc_R2K(norb,ns,nr,nk,r,k,matr(:,:,:,:,ifreq),matk(:,:,:,:,ifreq))
      end do

  end subroutine FPathDyn_R2K

  subroutine BPathStc_R2K(norb,ns,nr,nk,r,k,matr,matk)
      implicit None
      integer, intent(in) :: norb, ns, nr, nk
      real*8, intent(in) :: r(nr,3), k(nk,3)
      complex*16, intent(in) :: matr(norb,norb,ns,ns,nr)
      complex*16, intent(out) :: matk(norb,norb,ns,ns,nk)

      integer :: iorb, jorb, is, js, ir, ik
      double precision :: pi
      complex*16 :: tempmat(nr), ai, tempval
            ...
            do js = 1, ns
               do iorb = 1, norb
                  do jorb = 1, norb
                     tempval = 0.0d0
                     do ir = 1, nr
                        tempval = tempval + matr(iorb,jorb,is,js,ir)*cdexp(-2.0d0*ai*pi*dot_product(k(ik,:),r(ir,:)))
                     enddo
                     matk(iorb,jorb,is,js,ik) = tempval
                  enddo
               enddo
            enddo
         enddo
      enddo
  end subroutine BPathStc_R2K

  subroutine BPathDyn_R2K(norb,ns,nr,nk,nfreq,r,k,matr,matk)
      implicit None
      integer, intent(in) :: norb, ns, nr, nk, nfreq
      real*8, intent(in) :: r(nr,3), k(nk,3)
      complex*16, intent(in) :: matr(norb,norb,ns,ns,nr,nfreq)
      complex*16, intent(out) :: matk(norb,norb,ns,ns,nk,nfreq)

      integer :: ifreq

      do ifreq = 0, nfreq
          call BPathStc_R2K(norb,ns,nr,nk,r,k,matr(:,:,:,:,:,ifreq),matk(:,:,:,:,:,ifreq))
      enddo

  end subroutine BPathDyn_R2K
