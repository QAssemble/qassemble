...
class CorrelationFunction(object):
    ...
    def GW_EDMFT(self, itermax, imp, equiv, gint : GreenInt=None, vloc : VLoc=None): ### gint should be class GreenInt and vloc should be class VLoc(BLocStc)
        ...
        for iter in range(1,itermax): ### should be starting from 1, since 0th iteration has already been done above
            ...
            sigmaf_dc,sigmac_dc,pi_dc,wloc,gloc = self.compute_DC_part(gint_edmft,vloc)
            #### EDMFT part ####
            for key in range(1,nprob+1):
    ...
    ### used for GW_EDMFT
    def compute_DC_part(self,gint,vloc):
        ...
        return sigmaf_dc,sigmac_dc,pi_dc,wloc,gloc
        

    ### used for GW_EDMFT
    def compute_IMP_part(self,iter,key,imp,equiv,gint,gloc,vloc,wloc,pi_dc,sigmaf_dc,sigmac_dc,ctqmc): ### used for GW_EDMFT
        ...
        weiss_green = FWeiss(crystal=self.crystal,ft=self.ft,niham=self.niham.k,mu=gint.mu,hamh=self.sigmah.k,hamf=self.sigmaf.k,hloc=sigmah_dc.r,floc=sigmaf_dc.r,gloc=gloc.gf,sigmahimp=sigmah_dc.r,sigmafimp=sigmaf_dc.r,sigmacimp=sigmac_dc.rf)
