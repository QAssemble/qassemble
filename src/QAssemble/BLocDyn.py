...
class BLocDyn(object):
    ...
    def _as_dyn_dict(self, key = None, dyn : dict = None) -> dict:
        ...
        # Single source with the utilde served to PImp/WImp: the scalar the
        # solver receives is exactly the one f_to_solver reconstructs from.
        avg = getattr(self, "cf_to_solver_uniform", None)
        if avg is None:
            avg = getattr(self, "cf_to_solver", None)
        if avg is not None:
            return {"1": np.asarray(avg).real.tolist()}
...
class BWeiss(BLocDyn):
    ...
    def _density_boson_indices(self, pkey) -> list:
        ...
        return [
            self.projector.ProbFPair2Borb(pkey, iorb, iorb)
    ...
    def _dynamic_average(self, key, mat : np.ndarray) -> np.ndarray:
        ...
        nfreq = mat.shape[4]
        pkey = self.ResolveProblemKey(key)
        density_b = self._density_boson_indices(pkey)
    ...
    def _build_solver_consistent(self) -> None:
        """Rebuild the solver-consistent interaction served to PImp/WImp.

        dyn.json collapses the correlated bath to one scalar D(nu) coupled to
        the total density, so the impurity chi was generated by v + D(nu) on
        every density pair — not by the matrix-valued Weiss field.  P =
        -chi (1 - U chi)^-1 only removes the reducible ladders if U is that
        same interaction, so PImp/WImp must amputate with this reconstruction
        (gw_edmft_v208 edmft.py:2727, FullGWEDMFT Boson.F read_Uijkl_gname),
        not with the full matrix ``f``.
        """
        pkey = self.key
        self.cf_to_solver = self._dynamic_average(pkey, self.cf)
        self.cf_to_solver_uniform = self._dynamic_average(pkey, self.cf_uniform)

        v = np.asarray(self.vloc.vproj[pkey], dtype=np.complex128)
        density_b = self._density_boson_indices(pkey)
        ix = np.ix_(density_b, density_b)

        def _assemble(scalar):
            f = np.repeat(v[..., np.newaxis], scalar.shape[-1], axis=4)
            f[ix] += scalar[np.newaxis, np.newaxis, np.newaxis, np.newaxis, :]
            return np.asfortranarray(f)

        self.f_to_solver = _assemble(self.cf_to_solver)
        self.f_to_solver_uniform = _assemble(self.cf_to_solver_uniform)
