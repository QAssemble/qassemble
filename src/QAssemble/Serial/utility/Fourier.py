...
class Fourier:
    ...
    """
    @staticmethod
    def FLocDynT2F(ftau : ArrayLike, d : dlr, beta : np.float64) -> ArrayLike:

        norb, _, ns, _ = ftau.shape
        nfreq = len(d.get_matsubara_frequencies(beta))

        ff = np.zeros((norb, norb, ns, nfreq), dtype=np.complex128, order='F')

        for js in range(ns):
            tempmat = ftau[:, :, js, :].T
            fxx = d.dlr_from_tau(G_laa=tempmat)
            ff[:, :, js, :] = d.matsubara_from_dlr(G_xaa=fxx, beta=beta, xi=-1).T

        return ff
    
    @staticmethod
    def FLatDynT2F(ftau : ArrayLike, d : dlr, beta : np.float64) -> ArrayLike:

        norb, _, ns, nrk, _ = ftau.shape
        nfreq = len(d.get_matsubara_frequencies(beta))

        ff = np.zeros((norb, norb, ns, nrk, nfreq), dtype=np.complex128, order='F')

        for ik in range(nrk):
            ff[..., ik, :] = Fourier.FLocDynT2F(ftau[..., ik, :], d=d, beta=beta)
