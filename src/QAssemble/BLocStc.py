...
class BLocStc(object):
    ...
    def Mixing(self, iter : int, mix : float, Bb : np.ndarray, Bold : np.ndarray)-> np.ndarray:
        ...
        Bnew = np.zeros((norb,norb,ns,ns),dtype=np.complex128)

        if iter == 1:
            mix = 1.0
            Bold = np.zeros((norb,norb,ns,ns),dtype=np.complex128)
    ...
    def Imp2Loc(self,matimp : np.ndarray)-> np.ndarray:
        ...
        matloc = np.zeros((norb,norb,ns,ns,nspace),dtype=np.complex128)
    ...
    def Dict2Arr(self, equiv : np.ndarray, matdict : dict) -> np.ndarray:
        ...
        norb = len(equiv)
        ns = self.crystal.ns
        matout = np.zeros((norb,norb,ns),dtype=np.complex128)
        nind = np.amax(equiv)
    ...
    def Double2Quad(self, matin):
        ...
        matout = np.zeros((norbc,norbc,norbc,norbc,ns,ns),dtype=np.complex128)
...
class VLoc(BLocStc):
    ...
    def __init__(self, crystal: Crystal,voption : dict = None):
        ...
        self.vloc = np.zeros((norb,norb,ns,ns),dtype=float)
        if voption is None:
    ...
    def GenOnsite(self):
        ...
        tempmat = np.zeros((norbc,norbc,norbc,norbc,ns,ns),dtype=np.complex128)
    ...
    def KanamoriParameter(self, norb : int, val : list) -> np.ndarray:
        ...
        ns = self.crystal.ns
        v = np.zeros((norb,norb,norb,norb,ns,ns),dtype=float)
        U = val[0]
        Up = val[1]
        J = val[2]
    ...
    def SlaterParameter(self, l : int = None,norbc : int=None, val : list=None, sc : str = 'c') -> np.ndarray:
        ...
        ns = self.crystal.ns
        norb = 2*l+1
        vtemp = np.zeros((norb,norb,norb,norb,ns,ns),dtype=float)
        v = np.zeros((norbc,norbc,norbc,norbc,ns,ns),dtype=float)
    ...
    def Spherical2Cubic(self,v : np.ndarray,l : int):
        ...
        V = np.array(tempmat,dtype=float)
    ...
    def GetUijklComCTQMC(self, key):
        ...
        norbc = len(orb)
        tempmat = np.zeros((norb, norb, norb, norb), dtype=np.complex128)
        vloc = np.zeros((norbc, norbc, norbc, norbc, ns, ns), dtype=np.complex128)
        for ks in range(ns):
            for js in range(ns):
        ...
        if (self.crystal.soc == False):
            U = np.zeros((norbc**4*2**4), dtype=np.float64)
            idx = 0
            if (ns == 1):
                for sl in range(2):
