...
class FPathStc(object):
    ...
    def Inverse(self, mat: np.ndarray):
        ...
        matinv = np.zeros((norb, norb, ns, nrk), dtype=np.complex64)
    ...
    def FermiSurface(self, hmat : np.ndarray = None, num : int = 101):
        ...
        fs = fs.reshape((norb,norb,ns,num,num,1))
        kpoint = kpoint.reshape((num,num,1,3))
    ...
    def Occ(self, hmat : np.ndarray = None, beta : float = None):
        ...
        occk = np.zeros((norb, norb, ns, nk),dtype=np.complex128)
    ...
    def SlabKpoint(self):
        ...
        kpoint = np.zeros((kgrid[0], kgrid[1], kgrid[2], 3))
    ...
    def Reshape(self, matk : np.ndarray = None, kpoint : np.ndarray = None):
        ...
        (norb, norb, ns, nk) = matk.shape
        matkret = np.zeros((norb, norb, ns, kgrid[0], kgrid[1], kgrid[2]), dtype=np.complex128)
    ...
    def Moments(self, matk : np.ndarray = None, beta : np.float64 = None, kgrid : list = None):
        ...
        n = np.zeros((norb, ns, 2), dtype=np.complex128)
