...
tempmat = np.zeros([norb,norb],dtype=complex)
tempmat1 = np.zeros([norb,norb],dtype=complex)
tempmat2 = np.zeros([norb,norb],dtype=complex)
eig = np.zeros([norb],dtype=complex)
eigmat = np.zeros([norb,norb],dtype=complex)
hmat = np.zeros([norb,norb,ns,nk],dtype=complex)
identity = np.identity(norb,dtype=complex)
identity = np.array(identity)
glatt0 = np.zeros([norb,norb,ns,nk,nomega],dtype=complex)
flatdyn_t = np.zeros([norb,norb,ns,nk,ntau],dtype=complex)
flatdyn_f = np.zeros([norb,norb,ns,nk,nomega],dtype=complex)
flatdyn_moment = np.zeros([norb,norb,ns,nk,3],dtype=complex)
flatdyn_high = np.zeros([norb,norb,ns,nk],dtype=complex)

omega = np.zeros([nomega],dtype=np.float64)
