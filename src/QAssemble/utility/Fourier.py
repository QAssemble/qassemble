...
class Fourier:
    ...
    @staticmethod
    def FLatKR(rkgrid : list, fin : np.ndarray, sign : int, norm : float) -> np.ndarray:

        fout = np.zeros_like(fin, dtype=np.complex128)

        norb, _, ns, nrk = fin.shape

        for js in range(ns):
            for iorb, jorb in itertools.product(range(norb), repeat=2):
                tempmat = fin[iorb, jorb, js, :].copy()
                Common.fftw3_3d(tempmat, sign)
    ...
    @staticmethod
    def FLatStcK2R(fin : np.ndarray, mpimanager : MPIManager) -> np.ndarray:
