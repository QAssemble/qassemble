"""This module provides a collection of functions to calculate bare Green's functions
for fermionic and bosonic systems in various representations (frequency/imaginary time,
local/lattice).

A collection of functions to compute bare Green's functions for non-interacting systems.
...
def FLatFreq(freq : np.ndarray, hlatt : np.ndarray) -> np.ndarray:
    ...
    for ik in range(nk):
        glatt[..., ik, :] = FLocFreq(freq, hlatt[..., ik])
...
def FLocTau(tau : np.ndarray, beta : np.float64,  hloc : np.ndarray) -> np.ndarray:
    ...
    for js in range(ns):
        ...
        for iorb in range(norb):
            gtau[:, iorb] = FTau(tau, beta, w[iorb])
...
def FLatTau(tau : np.ndarray, beta : np.float64, hlatt : np.ndarray) -> np.ndarray:
    ...
    for ik in range(nk):
        glatt[..., ik, :] = FLocTau(tau, beta, hlatt[..., ik])
...
def BLatFreq(freq : np.ndarray, hlatt : np.ndarray) -> np.ndarray:
    ...
    for ik in range(nk):
        wlatt[..., ik, :] = BLocFreq(freq, hlatt[..., ik])
