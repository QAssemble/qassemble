...
class Bare:
    ...
    def FFreq(freq : np.ndarray, energy : np.ndarray) -> np.ndarray:
        ...
        nfreq = len(freq)
        gfreq = np.ndarray((nfreq), dtype=np.ndarray)
    ...
    def FTau(tau : np.ndarray, beta : np.float64, energy : np.ndarray) -> np.ndarray:
        ...
        ntau = len(tau)
        gtau = np.ndarray((ntau), dtype=np.complex128)
    ...
    def BFreq(freq : np.ndarray, energy : np.ndarray) -> np.ndarray:
        ...
        nfreq = len(freq)
        wfreq = np.zeros((nfreq), dtype=np.complex128)
    ...
    def BTau(tau : np.ndarray, energy : np.ndarray) -> np.ndarray:
        ...
        ntau = len(tau)
        wtau = np.zeros((ntau), dtype=np.complex128)
    ...
    def FLocFreq(freq : np.ndarray, hloc : np.ndarray)->np.ndarray:
        ...
        nfreq = len(freq)
        tempmat = np.zeros((norb, norb), dtype=np.complex128)
        tempmat2 = np.zeros((norb, norb), dtype=np.complex128)
        gloc = np.zeros((norb, norb, ns, nfreq), dtype=np.complex128)

        for js in range(ns):
            tempmat = hloc[:, :, js]
            w, v = Common.HermitianEigenCmplx(tempmat)
            gfreq = np.zeros((nfreq, norb), dtype=np.complex128)
    ...
    def FLatFreq(freq : np.ndarray, hlatt : np.ndarray) -> np.ndarray:
        ...
        nfreq = len(freq)
        glatt = np.zeros((norb, norb, ns, nk, nfreq),dtype=np.complex128)
    ...
    def FLocTau(tau : np.ndarray, beta : np.float64,  hloc : np.ndarray) -> np.ndarray:
        ...
        tempmat = np.zeros((norb, norb), dtype=np.complex128)
        tempmat2 = np.zeros((norb, norb), dtype=np.complex128)
        gloc = np.zeros((norb, norb, ns, ntau), dtype=np.complex128)

        for js in range(ns):
            tempmat = hloc[:, :, js]
            w, v = Common.HermitianEigenCmplx(tempmat)
            gtau = np.zeros((ntau, norb), dtype=np.complex128)
    ...
    def FLatTau(tau : np.ndarray, beta : np.float64, hlatt : np.ndarray) -> np.ndarray:
        ...
        norb, _, ns, nk = hlatt.shape
        ntau = len(tau)
        glatt = np.zeros((norb, norb, ns, nk, ntau),dtype=np.complex128)
    ...
    def BLocFreq(freq : np.ndarray, hloc : np.ndarray) -> np.ndarray:
        ...
        tempmat = np.zeros((norb, norb), dtype=np.complex128)
        tempmat2 = np.zeros((norb, norb), dtype=np.complex128)
        wloc = np.zeros((norb, norb, ns, ns, nfreq), dtype=np.complex128)

        for ks in range(ns):
            for js in range(ns):
                tempmat[:, :, js, ks] = hloc[:, :, js, ks]
                w, v = Common.HermitianEigenCmplx(tempmat)
                wfreq = np.zeros((nfreq, norb), dtype=np.complex128)
    ...
    def BLatFreq(freq : np.ndarray, hlatt : np.ndarray) -> np.ndarray:
        ...
        nfreq = len(freq)
        wlatt = np.zeros((norb, norb, ns, ns, nk, nfreq), dtype=np.complex128)
    ...
    def BLocTau(tau : np.ndarray, hloc : np.ndarray) -> np.ndarray:
        ...
        tempmat = np.zeros((norb, norb), dtype=np.complex128)
        tempmat2 = np.zeros((norb, norb), dtype=np.complex128)
        wloc = np.zeros((norb, norb, ns, ns, ntau), dtype=np.complex128)

        for ks in range(ns):
            for js in range(ns):
                tempmat = hloc[:, :, js, ks]
                w, v = Common.HermitianEigenCmplx(tempmat)
                wtau = np.zeros((ntau, norb), dtype=np.complex128)
    ...
    def BLatTau(tau : np.ndarray, hlatt : np.ndarray) -> np.ndarray:
        ...
        norb, _, ns, _, nk = hlatt.shape
        ntau = len(tau)
        wlatt = np.zeros((norb, norb, ns, ns, nk, ntau), dtype=np.complex128)
