...
class MPIManager(object):
    ...
    def Query(self, **kwargs):
        ...
        nk = kwargs['nk']
        nfermion = kwargs['nfermion']
        nboson = kwargs['nboson']
        shape = kwargs['shape']

        if (nk, nfermion, nboson, nprock, nprocf) in self.mpidict:
            return self.mpidict[(nk, nfermion, nboson, nprock, nprocf)]
        else:
            nodedict = {}
            ...
            # ftemp = np.arange(nf)
            fermiontemp = np.arange(nfermion)
            fchunk = np.array_split(fermiontemp, nprocf)
            submatrixfermion = [(chunk[0], chunk[-1]+1) for chunk in fchunk]
            nodedict['submatrixfermion'] = submatrixfermion
            floc = self.mf.FTLocalGlobal(submatrixfermion)
            self.mf.floc = floc
            
            
            bosontemp = np.arange(nboson)
            ...
            commfermion = self.comm.Split(color=fidx, key=kidx)
            commboson = self.comm.Split(color=fidx, key=kidx)
            ...
            nodedict['commfermion'] = commfermion
            nodedict['commboson'] = commboson
            ...
            self.mpidict[(nk, nfermion, nboson, nprock, nprocf)] = nodedict
...
class MPIFunction():
    ...
    def KRVec2Idx(self, rank, matin : np.ndarray, kloc : dict) -> np.ndarray:

        (nkx, nky, nkz) = matin.shape

        if (nkx*nky*nkz != len(kloc[rank])):
            print(self.KRVec2Idx.__name__)
            print("Input array is wrong. Check the array dimension")
            print(matin.shape, nkx, nky, nkz)
            sys.exit()
