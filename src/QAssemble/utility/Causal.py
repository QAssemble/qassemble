...
class CausalCheckResult:
    ...
    ``node_residual`` is filled by the kernel-aware checks
    (``CausalFermion``/``CausalBoson``) as a data-quality diagnostic; the
    pure-QP ``CausalProjection.check`` leaves it ``None``.
    """
...
class _CausalBase:
    """Shared decompose -> gate -> check -> QP -> revalidate pipeline.

    Subclasses own the statistics: the kernel sign ``xi``, the moment rows,
    and the kernel construction.  The QP itself is delegated to a composed
    ``CausalProjection`` instance (``self.qp``).

    The pipeline for one scalar Matsubara channel is:

    1. scale the target by ``max(|target|, 1)``;
    2. decompose into real DLR pole weights via stacked real/imag least
       squares on the native node kernel;
    3. gate: a node fit residual above ``fit_tol`` means the target is not
       representable by the real-coefficient pole basis -> ``RuntimeError``;
    4. compute the equality target as the moments of the *fitted* reference
       coefficients (moments are not externally specifiable);
    5. if the reference is already causal, skip the QP (the output is still
       the refit ``kernel @ reference``);
    ...
    def project(self, target: np.ndarray) -> np.ndarray:
        ...
        reference, node_residual = self._fit_coefficients(target_scaled)
        try:
            self._gate(node_residual)
        except RuntimeError:
            self.last_status = "gate_failed"
        ...
        normal_eq_residual = float(
            np.linalg.norm(
                ...
                - np.real(self.kernel.conj().T @ target_scaled)
            )
        )

        equality_target = self.moment_rows @ reference
        tol_scaled = self.constraint_tol / scale
        ...
        return self.kernel @ coefficients
    ...
    def _validate_target(self, target_vec: np.ndarray) -> None:
        """Subclass hook for statistic-specific input validation."""
        ...
        *,
        ...
        return {
            "valid": bool(
                ...
                and coefficient_violation <= self.constraint_tol
                and moment_residual <= self.constraint_tol
...
class CausalFermion(_CausalBase):
    """Fermionic causal projection of scalar Matsubara channels.

    Uses the xi=-1 DLR kernel on the native Matsubara nodes, enforces
    nonpositive pole weights (``coefficient_sign=-1``), and preserves the
    p = 0, 1, 2 frequency moments of the fitted reference coefficients.
    """
...
class CausalBoson(_CausalBase):
    """Bosonic causal projection of scalar Matsubara channels.

    Uses the xi=+1 DLR kernel on the native bosonic nodes.  The tanh factor
    of the bosonic kernel absorbs the sign structure of the odd spectral
    function, so a uniform coefficient sign (default ``A_l <= 0``) suffices.

    With ``reflection_symmetry=True`` (default, matching
    causal_boson.BosonPoleQPProjector) the kernel is symmetrized,
    ``K_sym = (K(+i nu) + K(-i nu)) / 2``, and only the M2 moment row
    ``tanh(x/2) * omega_l`` is enforced; with ``False`` the plain kernel is
    used and both M1 = ``tanh(x/2)`` and M2 rows are enforced.  The tanh
    argument is always the dimensionless DLR node ``x``, never
    ``beta * omega``.

    The caller must isolate the dynamic part first (``W = V + W_dyn``): a
    constant offset is not representable by decaying poles.  Targets that do
    not decay at the largest ``|nu|`` nodes are rejected with a
    ``RuntimeError`` (zeroing only the ``nu = 0`` node is insufficient);
    ``tail_tol`` controls this guard.

    Caveat for ``reflection_symmetry=True``: the symmetrized kernel is
    numerically rank-deficient (effective rank ~ half), so the coefficient
    representation of a given target is not unique.  ``check`` evaluates the
    sufficient sign condition on the least-squares representative and can
    therefore report non-causal for data that admits a causal representation
    (e.g. the output of ``project``, whose causal coefficients are in
    ``last_coefficients``).  ``project`` itself is unaffected.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # set before super().__init__, which builds the kernel/moment rows
    ...
    def _moment_rows(self) -> np.ndarray:
        # dimensionless x in the tanh — tanh(0.5 * beta * omega_l) in
        ...
        row_m1 = self.bose_corr
        return np.vstack([row_m1, row_m2])
