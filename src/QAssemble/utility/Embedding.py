...
def FLocStc(
    ...
    """
    norb, ns, _ = _validate_fe(ffc, projector, ffc_ndim=3)
    result = _allocate_output((norb, norb, ns), ffc, out)
...
def FLatStc(
    ...
    result = _allocate_output((norb, norb, ns, nk), ffc, out)
    local = FLocStc(ffc, projector)
...
def FLocDyn(
    ...
    """
    norb, ns, _ = _validate_fe(ffc, projector, ffc_ndim=4)
    nf = ffc.shape[3]
    result = _allocate_output((norb, norb, ns, nf), ffc, out)

    for ifreq in range(nf):
        result[:, :, :, ifreq] = FLocStc(ffc[:, :, :, ifreq], projector)
...
def FLatDyn(
    ...
    nf = ffc.shape[3]
    result = _allocate_output((norb, norb, ns, nk, nf), ffc, out)

    for ifreq in range(nf):
        result[:, :, :, :, ifreq] = FLatStc(
...
def BLocStc(
    ...
    """
    norb, ns, _ = _validate_be(ffc, projector, ffc_ndim=4)
    result = _allocate_output((norb, norb, ns, ns), ffc, out)
...
def BLatStc(
    ...
    result = _allocate_output((norb, norb, ns, ns, nk), ffc, out)
    local = BLocStc(ffc, projector)
...
def BLocDyn(
    ...
    """
    norb, ns, _ = _validate_be(ffc, projector, ffc_ndim=5)
    nf = ffc.shape[4]
    result = _allocate_output((norb, norb, ns, ns, nf), ffc, out)

    for ifreq in range(nf):
        result[:, :, :, :, ifreq] = BLocStc(
            ffc[:, :, :, :, ifreq], projector
        )
...
def BLatDyn(
    ...
    nf = ffc.shape[4]
    result = _allocate_output((norb, norb, ns, ns, nk, nf), ffc, out)

    for ifreq in range(nf):
        result[:, :, :, :, :, ifreq] = BLatStc(
            ffc[:, :, :, :, ifreq], projector, nk
        )
...
Embedding = _sys.modules[__name__]
