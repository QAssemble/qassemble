...
! private :: gcoeff
  public :: dcmplx_matinv, indexing, fderiv_dcmplx,spline_dcmplx, fftw3_1d, fftw3_2d, fftw3_3d, factorial_int, &
            hermitianeigen_dcmplx, BernoulliPolynomial, EulerPolynomial, ttind, Legendre2Chebyshev,gcoeff, min_distance
...
subroutine min_distance(R, S, d)
  ...
  double precision :: Rtemp, R1, R2, dtemp(3), rr(3,3)
  integer :: ix,iy,iz
  ...
  do ix = -1, 1
    do iy = -1, 1
      do iz = -1, 1
        ...
        rr(2,:) = iy * S(2,:)
        rr(3,:) = iz * S(3,:)
        dtemp = d - rr(1,:) - rr(2,:) - rr(3,:)

        R2 = sqrt(dtemp(1)**2 + dtemp(2)**2 + dtemp(3)**2)
