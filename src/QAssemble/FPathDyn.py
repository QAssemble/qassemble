...
class FPathDyn(object):
    ...
    def Inverse(self, mat: np.ndarray) -> np.ndarray:
        ...
        matinv = np.zeros((norb, norb, ns, nrk, nft), dtype=np.complex64)
    ...
    def KArb(self, matk: np.ndarray = None, kpoint: np.ndarray = None, omega : np.ndarray = None):  ## naming
        ...
        tempmat = np.zeros((norb, norb, ns, nk, nfreq), dtype=complex)
        ...
        matkinv = np.zeros((norb, norb, ns, nkpath, nfreq), dtype=complex)
        for ifreq in range(nfreq):
    ...
    def  MQEMWrapper(self, option: dict = None):
        ...
        gmat = np.zeros((norb, norb, ns, 1, nomega), dtype=np.complex128)
        gauxmat = np.zeros((norb, norb, ns, 1, nomega), dtype=np.complex128)
        ...
        for ik in range(1):
            for js in range(ns):
                for iorb in range(norb):
                    ...
                    logger.info(
                        "iorb %s center: %s width: %s", iorb + 1, acenter, awidth
                    )
                    emax = max(round(abs(acenter) + awidth * 30), emax)
        # emax = 10
        logger.info("emax: %s", emax)
        logger.debug("")
        ...
        for ik in range(1):
            for js in range(ns):
                for iorb in range(norb):
                    ...
                    if iorb == 0:
                        sig_real = np.zeros(
                            (norb, norb, ns, 1, nf), dtype=np.complex128
                        )
                    if gauxmode == "asitis":
        ...
        if interpolation:
            ...
            sig_real = np.zeros(
                (norb, norb, ns, 1, 2 * nn + 1), dtype=np.complex128
            )
    ...
    def Spectral(self, option: dict = None):
        ...
        if gauxmode == "auxg":
            ...
            # kf = np.zeros_like(tempmat3,dtype=np.complex128)
        ...
        sigout = np.zeros((ns, nk, nf), dtype=np.complex128)
        gout = np.zeros((ns, nk, nf), dtype=np.complex128)
