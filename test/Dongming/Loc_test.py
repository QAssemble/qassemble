...
from qacore.FLatStc import FLatStc,NIHamiltonian,Hamiltonian,SigmaHartree,SigmaFock,SigmaHartree_k
...
RVec = [[1,0,0],[1/2,np.sqrt(3)/2,0],[0,0,10]]
Basis = [[[1/3,1/3,0],1],[[2/3,2/3,0],1]]
NSpin = 1
SOC = False
KGrid = [15,15,1]
# KGrid = [30,30,1]
...
flatstc=FLatStc(crystal=cr)
niham = NIHamiltonian(crystal=cr,hopping=hopping)
...
kpath = [[0,0,0],[2/3,1/3,0],[1/2,0,0],[0,0,0]]
nkpath = 101
fpathstc.crystal.Kpath(kpath, nkpath)
hmatk = fpathstc.R2K(matr=cf.niham.r, kpoint=fpathstc.crystal.kpath)
print(hmatk[:,:,0,0])
...
# cf.HartreeFock(itermax = itermax, mix = mix, hopping=hopping, loccoulomb = locoption, nonloccoulomb=nonlocoption)
# fpathstc.Band(hmat = cf.ham.r, plotoption=True)
...
# from qacore.FLocStc import FLocStc,SigmaHLoc,SigmaFLoc
# from qacore.BLocDyn import BLocDyn,PolLoc,WLoc

# gint = cf.green
# cf.crystal.Projector(impdict={"1" : [[[0,0]],[[1,0]]]})
...
#         for jorb in range(1):
#             for iorb in range(1):
#                 err = g_average[iorb, jorb, js, 0, iff] - gloc.gf[iorb, jorb, js, iff, 0]
#                 if (abs(err) > 1.0e-6):
#                     print(iorb, jorb, js, iff, abs(err), g_average[iorb, jorb, js, 0, iff], gloc.gf[iorb, jorb, js, iff, 0])
...
# hlat = SigmaHartree(crystal=cf.crystal,occ=gint.occ,vbare=cf.vbare.k)
...
# for iff in range(nf):
#     for ik in range(nk):
#         hartree_average[...,iff] += 1/nk*hlat.k[...,ik]
...
#                     err = pollat.rf[iorb,jorb,js,ks,0,iff] - polloc.rf[iorb,jorb,js,ks,iff,0]
#                     if (abs(err) > 1.0e-6):
#                         print(iorb, jorb, js, ks, iff, abs(err), pollat.rf[iorb,jorb,js,ks,0,iff], polloc.rf[iorb, jorb, js, ks, iff, 0])
...
# plot = plt.figure(1)
# # plt.scatter(cf.ft.nu[:],wlat_average.crf[0,0,0,0,0,:],color='blue')
...
plot = plt.figure(1)
plt.scatter(cf.ft.omega[:], 1/delta.rf[0, 0, 0, :, 0], color="blue")
plt.legend()
plt.grid(which="both", linestyle="--", linewidth=0.3)
plt.show()