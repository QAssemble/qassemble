...
def main():
    ...
    glatm, high = flatdyn.Moment(glatt0, 1, 1)
    ...
    glatm2, high2 = flatdynmpi.Moment(ff=tempmat2, isgreen=True, highzero=True)
            ...
            for js in range(ns):
                for jorb in range(norb):
                    for iorb in range(norb):
                        ...
                        err = (
                            glatm[iorb, jorb, js, kidx, ii]
                            - glatm2[iorb, jorb, js, ik, ii]
                        )
                        if abs(err) > 1.0e-6:
                            print(
                                comm.Get_rank(),
