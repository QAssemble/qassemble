...
def main():
            ...
            for js in range(ns):
                for jorb in range(norb):
                    for iorb in range(norb):
                        ...
                        tempmat2[iorb, jorb, js, ik, iw] = glatt0[iorb, jorb, js, kidx, fidx]
                        tempmat3[iorb, jorb, js, ik, iw] = sig[iorb, jorb, js, kidx, fidx]
    ...
    if (comm.Get_rank() == 0):
        print("Dyson equation test start")
            ...
            for js in range(ns):
                for jorb in range(norb):
                    for iorb in range(norb):
                        kidx = flatdynmpi.mpimanager.KLocal2Global([flatdynmpi.commk.Get_rank(), ik])
                        fidx = flatdynmpi.mpimanager.FLocal2Global([flatdynmpi.commw.Get_rank(), iw])
                        err = tempmat[iorb, jorb, js, kidx, fidx] - tempmat4[iorb, jorb, js, ik, iw]
                        if (abs(err) > 1.0e-6):
                            print(iorb, jorb, js, ik, iw, abs(err), tempmat[iorb, jorb, js, kidx, fidx], tempmat4[iorb, jorb, js, ik, iw])
    if (comm.Get_rank() == 0):
        print("Dyson equation test finish")
