...
def main():
    ...
    tempmat = np.zeros((norb, norb, ns, ns, nk, nw), dtype=np.complex128, order='F')
                ...
                for js in range(ns):
                    for jorb in range(norb):
                        for iorb in range(norb):
                            tempmat[iorb, jorb, js, ks, ik, iw] = (((iorb + 1) - (jorb + 1)) / 2.0 + (js + 1+ks)*0.1 + (crystal.kpoint[ik][0]+crystal.kpoint[ik][1]+crystal.kpoint[ik][2]) / 2.0 + (iw + 1) * 0.001)

    tempmat2 = blatdyn.Inverse(tempmat)
    tempmat = tempmat.reshape((norb, norb, ns, ns, 9, 9, 9, nw), order='F')

    tempmat3 = blatdynmpi.Inverse(tempmat)

    tempmat4 = np.zeros_like(tempmat, dtype=np.complex128, order='F')
    tempmat5 = np.zeros_like(tempmat, dtype=np.complex128, order='F')

    blatdynmpi.commw.Allreduce(tempmat3, tempmat4, op=MPI.SUM)
    blatdynmpi.commk.Allreduce(tempmat4, tempmat5, op=MPI.SUM)
    tempmat5 = tempmat5.reshape((norb, norb, ns, ns, nk, nw),order='F')
            ...
            for ik in range(nk):
                for ks in range(ns):
                    for js in range(ns):
                        for jorb in range(norb):
                            for iorb in range(norb):
                                err = tempmat2[iorb, jorb, js, ks, ik, iw] - tempmat5[iorb, jorb, js, ks, ik, iw]
                                if (abs(err) > 1.0e-6).all():
                                    print(iorb, jorb, js, ks, ik, iw, abs(err), tempmat2[iorb, jorb, js, ks, ik, iw], tempmat5[iorb, jorb, js, ks, ik, iw])
