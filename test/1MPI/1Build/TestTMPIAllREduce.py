...
def main():
    ...
    submatrixtau = flatdynmpi.submatrixtau[flatdynmpi.commtau.Get_rank()]
    ntauloc = submatrixtau[1] - submatrixtau[0]
    nkloc = len(flatdynmpi.mpimanager.klocal[flatdynmpi.commk.Get_rank()])

    tempmat2 = np.zeros((norb, norb, ns, nkloc, ntauloc), dtype=np.complex128, order="F")
            ...
            for js in range(ns):
                for jorb in range(norb):
                    for iorb in range(norb):
                        ...
                        tidx = flatdynmpi.mpimanager.TLocal2Global(
                            [flatdynmpi.commtau.Get_rank(), itau]
                        )
                        tempmat2[iorb, jorb, js, ik, itau] = glattau[
                            iorb, jorb, js, kidx, tidx
                        ]
    glattau2 = np.zeros((norb, norb, ns, nkloc, ntau), dtype=np.complex128, order='F')

    for ik in range(nkloc):
        for js in range(ns):
            for jorb in range(norb):
                for iorb in range(norb):
                    glattau2[iorb, jorb, js, ik] = flatdynmpi.mpimanager.TMPIAllreduce(flatdynmpi.commtau, tempmat2[iorb, jorb, js, ik], ntau)
            ...
            for js in range(ns):
                for jorb in range(norb):
                    for iorb in range(norb):
                        ...
                        err = glattau[iorb, jorb, js, kidx, itau] - glattau2[iorb, jorb, js, ik, itau]
                        if (abs(err) > 1.0e-6):
                            print(iorb, jorb, js, ik, itau, abs(err), glattau[iorb, jorb, js, kidx, itau], glattau2[iorb, jorb, js, ik, itau])
