...
def main():
    ...
    ffglob = np.zeros((norb, norb, ns, nkloc, nw), dtype=np.complex128, order='F')
    # fflast = flatdynmpi.mpimanager.FMPIBCast(flatdynmpi.commw, tempmat2, len(ftgrid.omega)-1)
    for ik in range(nkloc):
        for js in range(ns):
            for jorb in range(norb):
                for iorb in range(norb):
                    ffglob[iorb, jorb, js, ik] = flatdynmpi.mpimanager.FMPIAllreduce(flatdynmpi.commw, tempmat2[iorb,jorb, js, ik], nw)
            ...
            for js in range(ns):
                for jorb in range(norb):
                    for iorb in range(norb):
                        ...
                        err = glatt0[iorb, jorb, js, kidx, iw] - ffglob[iorb, jorb, js, ik, iw]
                        if (abs(err) > 1.0e-6):
                            print(comm.Get_rank(), iorb, jorb, js, ik, abs(err), glatt0[iorb, jorb, js, kidx, iw], ffglob[iorb, jorb, js, ik, iw])
